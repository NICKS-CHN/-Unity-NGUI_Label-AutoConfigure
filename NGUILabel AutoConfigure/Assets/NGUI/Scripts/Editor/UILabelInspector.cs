...
public class UILabelInspector : UIWidgetInspector
{
    ...
    private string FontDesc = null;
    private bool FontEnable = true;
    /// <summary>
    ...
    protected override bool ShouldDrawProperties()
    {
        ...
        EditorGUI.BeginDisabledGroup(!isValid);
        {
            ...
            //EditorGUI.BeginDisabledGroup(uiFont != null);
            //{
            GUILayout.BeginHorizontal();
            SerializedProperty fontProp = NGUIEditorTools.DrawProperty("Font ID", serializedObject, "mFontID", GUILayout.Width(142f));
            if (fontProp.intValue != NGUISettings.fontID)
            {
                NGUISettings.fontID = fontProp.intValue;
                serializedObject.ApplyModifiedProperties();
                serializedObject.Update();
            }
            EditorGUI.BeginChangeCheck();
            FontDesc = UILabelConfigHelper.UpdateUILabelByFontID(mLabel, NGUISettings.fontID);
            if (null == uiFont)
                FontEnable = string.IsNullOrEmpty(FontDesc);
            if (FontEnable)
                EditorGUILayout.LabelField("字体ID不存在");
            else
                EditorGUILayout.LabelField(FontDesc);
            GUILayout.EndHorizontal();
            ...
            if (dynFont != null)
            {
                GUILayout.BeginHorizontal();
                {
                    EditorGUI.BeginDisabledGroup(!FontEnable);
                    {
                        EditorGUI.BeginDisabledGroup((ttf != null) ? ttf.hasMultipleDifferentValues : fnt.hasMultipleDifferentValues);

                        SerializedProperty prop = NGUIEditorTools.DrawProperty("Font Size", serializedObject, "mFontSize",
